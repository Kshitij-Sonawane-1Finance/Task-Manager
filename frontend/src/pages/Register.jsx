  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.statusCode === 201) {
        ...
        setPopup({ show: true, success: false, message: response.data.message || 'Registration failed' })
        setTimeout(() => setPopup({ show: false, success: false, message: '' }), 3000)
      }
    } catch (err) {
      setPopup({ show: true, success: false, message: err.response?.data?.message || 'Registration failed' })
      setTimeout(() => setPopup({ show: false, success: false, message: '' }), 3000)
    }
  }
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        <div>
          ...
          <input
            type="text"
            id="name"
            className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900"
